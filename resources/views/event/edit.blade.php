...
@section('page-title')
    {{ trans('pages/event.edit.page_title') }}
@endsection

@section('page-content')
@include('layouts.breadcrum', ['title' => trans('pages/event.edit.breadcrumb')])
    			...
    			<div class="card">
    				<div class="card-body">
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                {!! Form::text('identification', old('identification') ?? $event->identification, ['class' => $errors->first('identification') ? 'form-control is-invalid' : 'form-control', 'required' => true]) !!}
    					...
    					<div class="form-group row">
    						...
    						<div class="col-sm-9">
    							{!! Form::text('name', old('name') ?? $event->name, ['class' => $errors->first('name') ? 'form-control is-invalid' : 'form-control', 'required' => true]) !!}
    							@error('name')
    							<div class="invalid-feedback">{{ $message }}</div>
    							@enderror
    						</div>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-sm-9">
                                {!! Form::email('email', old('email') ?? $event->email, ['class' => $errors->first('email') ? 'form-control is-invalid' : 'form-control', 'required' => true]) !!}
                                @error('email')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
    					...
    					<div class="form-group row">
    						...
    						<div class="col-sm-9">
    							{{ Form::textarea("address",  old('address') ?? $event->address, ['class' => $errors->first('address') ? 'form-control is-invalid' : 'form-control', "placeholder" => trans('pages/event.fields.address')]) }}
    							@error('address')
    							<div class="invalid-feedback">{{ $message }}</div>
    							@enderror
    						</div>
    				...
    				<div class="form-group row">
    					...
    					<div class="col-sm-7">
    						{!! Form::text('start_date', old('start_date') ?? $event->start_date_format, ['class' => $errors->first('start_date') ? 'form-control datetimepicker is-invalid' : 'form-control datetimepicker', 'placeholder' => trans('pages/event.fields.start_date'), 'required' => true]) !!}
    						@error('start_date')
    						<div class="invalid-feedback">{{ $message }}</div>
    						@enderror
    					</div>
    				...
    				<div class="form-group row">
    					...
    					<div class="col-sm-7">
    						{!! Form::text('end_date', old('end_date') ?? $event->end_date_format, ['class' => $errors->first('end_date') ? 'form-control datetimepicker is-invalid' : 'form-control datetimepicker', 'placeholder' => trans('pages/event.fields.end_date'), 'required' => true]) !!}
    						@error('end_date')
    						<div class="invalid-feedback">{{ $message }}</div>
    						@enderror
    					</div>
    				...
    				<div class="form-group row">
    					...
    					<div class="col-sm-7">
    						{!! Form::text('client', old('client') ?? $event->client, ['class' => $errors->first('client') ? 'form-control is-invalid' : 'form-control', 'placeholder' => trans('pages/event.fields.client'), 'required' => true]) !!}
    						@error('client')
    						<div class="invalid-feedback">{{ $message }}</div>
    						@enderror
    					</div>
    				...
    				<div class="form-group row">
    					...
    					<div class="col-sm-7">
    						{!! Form::text('phone', old('phone') ?? $event->phone, ['class' => $errors->first('phone') ? 'form-control is-invalid' : 'form-control', 'placeholder' => trans('pages/event.fields.phone'), 'required' => true]) !!}
    						@error('phone')
    						<div class="invalid-feedback">{{ $message }}</div>
    						@enderror
    					</div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            {!! Form::select('payment_method', ['' => __("select"), 'pago_movil' => "Pago movil", 'transferencia' => "Transferencia"], $event->payment_method ?? old('payment_method'), ['class' => 'form-control select2 hidden-search']) !!}
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-9">
                            {!! Form::text('reference_code', old('reference_code') ?? $event->reference_code, ['class' => $errors->first('reference_code') ? 'form-control is-invalid' : 'form-control', 'required' => true]) !!}
    				...
    				<div class="form-group row">
    					...
    					<div class="col-sm-7">
    						{{ Form::textarea("observations", old('observations') ?? $event->observations, ['class' => $errors->first('observations') ? 'form-control is-invalid' : 'form-control', "placeholder" => trans('pages/event.fields.observations'), 'rows' => 3]) }}
    						@error('observations')
    						<div class="invalid-feedback">{{ $message }}</div>
    						@enderror
    					</div>
...
@section('scripts')
	<script type="text/javascript">
        ...
        @elseif(!empty($event->products))
            @foreach($event->products as $product)
            $("select[name=product]").find('option:contains({{ $product->name }})').prop("disabled",true);
            @endforeach
        $('select[name=product]').prop('selectedIndex',0);
        $("select[name=product]").select2();
        @endif
